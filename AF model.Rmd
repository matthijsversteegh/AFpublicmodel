...
Probs <- function(l_params_all, M_t, df_X, t) { 
  ...
  with((l_params_all),{
    ...
    p_SFAF_SAF                                                            <- NULL
    p_SFAF_SAF[df_X$curTrt == "AAD" & df_X$Line == 1]                     <- ifelse(p_SFAF_CA_SAF_L1[t]*RR_nai>1, 1, p_SFAF_CA_SAF_L1[t]*RR_nai) 
    p_SFAF_SAF[df_X$curTrt == "AAD" & df_X$Line == 2 & df_X$AAD_exp == 0] <- ifelse(p_SFAF_CA_SAF_L2[t]*RR_CA_exp>1, 1, p_SFAF_CA_SAF_L2[t]*RR_CA_exp) 
    p_SFAF_SAF[df_X$curTrt == "AAD" & df_X$Line == 2 & df_X$AAD_exp == 1] <- ifelse(p_SFAF_CA_SAF_L2[t]*RR_AAD_exp>1, 1, p_SFAF_CA_SAF_L2[t]*RR_AAD_exp) 
    p_SFAF_SAF[df_X$curTrt == "AAD" & df_X$Line >= 3 & df_X$AAD_exp == 0] <- ifelse(p_SFAF_CA_SAF_L3[t]*RR_CA_exp>1, 1, p_SFAF_CA_SAF_L3[t]*RR_CA_exp) 
    p_SFAF_SAF[df_X$curTrt == "AAD" & df_X$Line >= 3 & df_X$AAD_exp == 1] <- ifelse(p_SFAF_CA_SAF_L3[t]*RR_AAD_exp>1, 1, p_SFAF_CA_SAF_L3[t]*RR_AAD_exp) 
    p_SFAF_SAF[df_X$curTrt == "no-treatment"]                             <- 1-p_D[df_X$curTrt == "no-treatment"] # get symptoms when no rhythm control treatment
    
    p_SFAF_SAF[df_X$curTrt == "CA" & df_X$Line == 1]                      <- p_SFAF_CA_SAF_L1[t]
    p_SFAF_SAF[df_X$curTrt == "CA" & df_X$Line == 2]                      <- p_SFAF_CA_SAF_L2[t] 
    p_SFAF_SAF[df_X$curTrt == "CA" & df_X$Line >= 3]                      <- p_SFAF_CA_SAF_L3[t] 
...
PrepareCosts <- function(l_params){
  ...
  with((l_params),{
    ...
    # Add productivity costs during recovery period to CA intervention costs
    c_CA                  <- unname(c_CA + c_prod_CA_recovery)
...
outcomes <- MicroSim(l_params, n_i, df_X, TRT1 = "AAD", TRT2 = "CA", TRT3 = "AAD", TRT4 = "CA", TRT5 = "AAD", TRT6 = "CA", seed = 1)
outcomes$te_hat         # Mean lifetime discounted QALYs
outcomes$tc_hat         # Mean lifetime discounted costs
outcomes$tLY_hat        # Mean lifetime discounted life years
outcomes$tLY_undisc_hat # Mean lifetime undiscounted life years
...
#report means by number of CA's
results_ce[results_ce$L1 == "CA",] %>% # use results_ce[results_ce$L1 == "CA",] for optimal n_CA's after CA in L1
  group_by(n_ca) %>%
    summarize(
      count = n(),
      mean_costs = mean(inc_Cost_vsAAD),
      mean_QALYs = mean(inc_QALYs_vsAAD),
      mean_NHB = mean(iNHB_vsAAD),
  # other summary statistics...
)

#means by CA in first line or not
results_ce %>%
  group_by(ca_first) %>%
    summarize(
      count = n(),
      ...
      mean_time = mean(Time_in_line1),
  # other summary statistics...
)

#mean time in treatment line
df_mean_time <- data.frame(line1CA  = mean(results_ce$Time_in_line1[results_ce$L1 == "CA"]),
                           line1AAD = mean(results_ce$Time_in_line1[results_ce$L1 != "CA"]),
                           line2CA  = mean(results_ce$Time_in_line2[results_ce$L2 == "CA"]),
                           line2AAD = mean(results_ce$Time_in_line2[results_ce$L2 != "CA"]),
                           line3CA  = mean(results_ce$Time_in_line3[results_ce$L3 == "CA"]),
                           line3AAD = mean(results_ce$Time_in_line3[results_ce$L3 != "CA"]),
                           line4CA  = mean(results_ce$Time_in_line4[results_ce$L4 == "CA"]),
                           line4AAD = mean(results_ce$Time_in_line4[results_ce$L4 != "CA"]),
                           line5CA  = mean(results_ce$Time_in_line5[results_ce$L5 == "CA"]),
                           line5AAD = mean(results_ce$Time_in_line5[results_ce$L5 != "CA"]),
                           line6CA  = mean(results_ce$Time_in_line6[results_ce$L6 == "CA"]),
                           line6AAD = mean(results_ce$Time_in_line6[results_ce$L6 != "CA"]))
...
#report means by number of CA's
results_ce_scenario %>% # use results_ce[results_ce$L1 == "CA",] for optimal n_CA's after CA in L1
  group_by(n_ca) %>%
    summarize(
      count = n(),
      mean_costs = mean(inc_Cost_vsAAD),
...
# 1: reading in the data and preparing it
# Read in PSA results
#psa_seq_results <- read.csv(here::here("output", "results_psa_ni5000_nsim5000.csv"))

#use code below to read the PSA data if it is run by the POSIT workbench
raw_lines <- readLines(here::here("output", "results_psa_ni5000_nsim1000_final.csv"))
...
psa_summary <- data.frame(cost_mean = aggregate(costs ~ treatment, psa_seq_results, mean),
                          ...
                          iNHB_mean  = aggregate(iNHB ~ treatment, psa_seq_results, mean),
                          iNHB_se    = aggregate(iNHB ~ treatment, psa_seq_results, sd))
                          #icost_mean = aggregate(incremental_cost ~ treatment, psa_seq_results, mean),
                          #ieffect_mean = aggregate(incremental_effect ~ treatment, psa_seq_results, mean))

psa_summary <- psa_summary[, -c(3,5,7,9,11,13)] # remove copies of treatment name columns
psa_summary <- psa_summary[, -c(3,5)]
colnames(psa_summary) <- c("Treatment_sequence", "Cost", "QALYs", "iNHB", "iNHB se" )

#2: #create dataframe for the comparison you want to make
# One comparison
df_psa1 <- psa_seq_results[psa_seq_results$treatment == "CA-AAD-AAD-AAD-AAD-CA",] # rank 1 in deterministic

# Multiple comparisons
df_psa2 <- rbind(psa_seq_results[psa_seq_results$treatment == "CA-AAD-AAD-AAD-AAD-CA",], psa_seq_results[psa_seq_results$treatment == "AAD-AAD-AAD-AAD-AAD-AAD",]) # top CA line 1 vs. top AAD line 1
...
ggplot(df_psa1, aes(x=incremental_effect, y=incremental_cost)) +
  geom_point(size = 1.2, shape = 16, color = "#42145f") +
  ...
  theme(plot.title = element_text(hjust = 0.5), 
        ...
        plot.caption = element_text(color = "#42145f", hjust = 0.5)) + 
  labs(title = "CE-plane",
       subtitle = "Most cost-effective 1st line AAD versus cost-effective 1st line CA",
       caption = "CA-AAD-AAD-AAD-AAD-CA vs AAD-AAD-AAD-AAD-AAD-CA")
ggsave(here::here("visual output", "Most cost-effective AAD vs CA.jpg"), width = 20, height = 12, units = c("cm"), dpi = 300)

# 4a Plot incremental results all treatments
ggplot(df_psa2, aes(x=incremental_effect, y=incremental_cost, colour=treatment)) +
  ...
  geom_hline(yintercept = 0)+
  geom_abline(intercept = 0, slope = 20000, linetype="dashed", lwd=1)+
  annotate("text", x = 0.9, y = 35000, label = "€20,000/QALY threshold")+
  theme_minimal()+
labs(title = "CE-plane", 
       subtitle = "Most cost-effective with CA or AAD in line 1 versus only AADs")
ggsave(here::here("visual output", "Most cost-effective CA or AAD in line 1 vs AADs.jpg"), width = 20, height = 12, units = c("cm"), dpi = 300)

# 4b Plot grouped comparison CA or AAD in line 1
...
# 4c Plot grouped comparison by number of CA's
ggplot(df_psa4, aes(x=incremental_effect, y=incremental_cost, colour=n_ca)) +
  geom_point(size = 1, shape = 16) +
  scale_y_continuous(name="", labels = dollar_format(prefix = "€", big.mark=".", decimal.mark = ",")) +
  ylab("Incremental Costs") +
  xlab("Incremental QALYs") +
  ...
  theme_minimal()+
labs(title = "CE-plane", 
       subtitle = "Grouped comparison by number of CA's in sequence")
ggsave(here::here("visual output", "Grouped comparison by number of CA's in sequence.jpg"), width = 20, height = 12, units = c("cm"), dpi = 300)
...
# 4d all results boxplot
ggplot(psa_seq_results, aes(x = treatment, y = NHB, fill = treatment)) +
  geom_boxplot(outlier.shape = NA)+
  scale_y_continuous(limits = quantile(psa_seq_results$NHB, c(0.1, 0.9))) +
  theme_minimal() +
  labs(title = "Net health benefit", 
       subtitle = "all treatments")

  ggsave(here::here("visual output", "all treatments.jpg"), width = 20, height = 12, units = c("cm"), dpi = 300)    
...
##Costs
psa_c_incr_L1_AAD <- df_psa2$incremental_cost[1:1000]
psa_c_incr_L1_CA <- df_psa2$incremental_cost[1001:2000]

##Effects
psa_e_incr_L1_AAD <- df_psa2$incremental_effect[1:1000]
psa_e_incr_L1_CA <- df_psa2$incremental_effect[1001:2000]

n_sim <- 1000
 
CEAC_L1_CA = function(x){
  sum((((x*psa_e_incr_L1_CA)-psa_c_incr_L1_CA) > 0)/n_sim)
}
 
CEAC_L1_AAD = function(x){
  sum((((x*psa_e_incr_L1_AAD)-psa_e_incr_L1_AAD) > 0)/n_sim)
}
    ...
    prob_L1_CA <- apply(thresholds, 1, CEAC_L1_CA)
    prob_L1_AAD <- apply(thresholds, 1, CEAC_L1_AAD)

 
# Combine data from line 1 and 2 into 1 dataframe    
CEAC_data <- data.frame("thresholds" = thresholds,
                        "prob_L1_CA" = prob_L1_CA,
                        "prob_L1_AAD" = prob_L1_AAD) 
CEAC_data_l  <- gather(CEAC_data, Line, prob, prob_L1_CA:prob_L1_AAD)
...
ggplot(data = CEAC_data_l,
       ...
       scale_x_continuous(breaks = seq(0, 200000, 50000), labels = dollar_format(prefix="€")) +
       theme_bw(base_size = 10)+
       #scale_color_manual(labels = c("Most cost-effective with CA in line 1", "Most cost-effective with AAD in line 1") + #, values = c("#EBDEF0", "#C39BD3")) +
...
## CEAC of all treatments
psa_seq_results <- read.csv(here::here("output", "results_psa_ni5000_nsim1000.csv"))

# Create a proper iteration index without using n()
psa_seq_results <- psa_seq_results %>%
...
reference_treatment_name <- "AAD-AAD-AAD-AAD-AAD-AAD" #Define reference treatment for PSA

# Extract reference values
reference_data <- psa_seq_results %>%
  dplyr::filter(treatment == reference_treatment_name) %>%
  dplyr::select(i, reference_cost = costs, reference_effect = effects)

# Now join with an inner join to ensure we only keep rows that match
psa_seq_results <- psa_seq_results %>%
  dplyr::inner_join(reference_data, by = "i") %>%
  dplyr::mutate(
    incremental_cost = costs - reference_cost,
    incremental_effect = effects - reference_effect,
    reference_treatment = reference_treatment_name
...
psa_seq_results <- psa_seq_results[psa_seq_results$treatment != reference_treatment_name, ]

which_psa <- psa_seq_results
...
colnames(ceac_threshold_res) <- c("Probability of being cost-effective versus only AADs", "Treatment_sequence")

ceac_threshold_res$`Probability of being cost-effective versus only AADs` <- ceac_threshold_res$`Probability of being cost-effective versus only AADs`*100
...
psa_results_with_prob <- inner_join(psa_summary, ceac_threshold_res, by = "Treatment_sequence")
View(psa_results_with_prob)

# Export results to Microsoft Word
library(flextable)
library(officer)

psa_results_with_prob <- psa_results_with_prob %>%
  arrange(desc(iNHB))

# Create a flextable object
ft <- flextable(psa_results_with_prob)
...
ft <- colformat_double(ft, j = "Probability of being cost-effective versus only AADs", digits = 1)   

# Save to Word document
doc <- read_docx()  # Create a new Word document
doc <- body_add_flextable(doc, value = ft)  # Add the table to the document
print(doc, target = here::here("output", "PSA_results.docx"))  # Save the document
